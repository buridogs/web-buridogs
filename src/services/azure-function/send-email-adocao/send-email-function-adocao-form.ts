...
export async function sendEmailFunctionAdocaoForm(adocaoData: IAdocaoPOSTRequestForm) {
    ...
    const response = await fetch(
        `https://serverless-email-buridogs.azurewebsites.net/api/SendEmailFunction?code=${process.env.NEXT_PUBLIC_AZURE_FUNCTION_KEY_SEND_EMAIL_FUNCTION_ADOCAO_FORM}`,
        {
            method: "POST",
